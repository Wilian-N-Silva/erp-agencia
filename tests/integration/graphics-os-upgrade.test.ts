  ...
  it("upgrades 0028 to client decisions and production without modifying an existing OS or document", async () => {
    ...
    try {
      await adminDb.transaction(async transaction => {
        ...
        for (let i = 29; i <= 36; i++) await applyMigration(transaction, i);
        ...
        expect((await transaction.execute(sql.raw("select count(*)::int n from graphic_supplier_commitments"))).rows).toEqual([{ n: 0 }]);
        expect((await transaction.execute(sql.raw("select count(*)::int n from graphic_sales"))).rows).toEqual([{ n: 0 }]);
        expect((await transaction.execute(sql.raw("select count(*)::int n from graphic_sale_installments"))).rows).toEqual([{ n: 0 }]);
