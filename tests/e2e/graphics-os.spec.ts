...
test("graphic flow from competing quotes and rejection to OS, production, reconciliation and closure", async ({ page }) => {
  ...
  await page.getByRole("textbox", { name: "Código interno", exact: true }).fill("QA-GRF-20260921");
  await page.getByRole("button", { name: "Criar trabalho", exact: true }).click();
  await expect(page.getByRole("alert").filter({ hasText: "Este código interno já está em uso" })).toBeVisible();
  await expect(page.getByRole("textbox", { name: "Título", exact: true })).toHaveValue("QA - Registro de OS externa");
  await expect(page.getByRole("textbox", { name: "Descrição", exact: true })).toHaveValue("Trabalho fictício para validar GRF-005.");
  await page.getByRole("textbox", { name: "Código interno", exact: true }).fill(code);
  await page.getByRole("button", { name: "Criar trabalho", exact: true }).click();
  ...
  await page.getByRole("textbox", { name: "Código interno", exact: true }).fill(code);
  await page.getByRole("button", { name: "Salvar alterações", exact: true }).click();
  await expect(page.getByRole("status").filter({ hasText: "Trabalho atualizado." })).toBeVisible();
