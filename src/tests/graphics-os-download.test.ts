...
vi.mock("@/features/graphics/os-dal", () => ({ getGraphicOsDownload: mocks.file }));
vi.mock("@/lib/storage", () => ({ getStorageObject: mocks.storage }));
vi.mock("@/lib/audit", () => ({ writeAuditLog: mocks.audit }));
import { GET } from "@/app/(private)/app/grafica/[id]/os/[versionId]/download/route";
...
const call = (jobId = id) => GET(request, { params: Promise.resolve({ id: jobId, versionId }) });
beforeEach(() => {
  vi.resetAllMocks();
  mocks.context.mockResolvedValue({ userId: "user", organizationId: "org" });
  mocks.file.mockResolvedValue({ id: "file", bucket: "private", storageKey: "secret-key", storageProvider: "local", originalName: "OS revisão.pdf" });
...
it("audits downloads and returns a private attachment without its storage key", async () => {
  ...
  expect(mocks.file).toHaveBeenCalledWith(expect.anything(), id, versionId);
