...
export default async function GraphicJobDetailPage({ params, searchParams }: {
  ...
  if (!job) notFound();
  const canEditQuotes = canWriteQuotes && ["supplier_sourcing", "supplier_approval_pending"].includes(job.operationalStatus);
  ...
  return <section className="flex w-full flex-col gap-6">
    ...
    {editing && options ? <Card title="Editar trabalho"><GraphicJobActionForm action={updateGraphicJobAction}><input name="id" type="hidden" value={id} /><GraphicJobFormFields job={job} options={options} /><div className="mt-5 flex justify-end"><button className={primaryButtonClassName} type="submit">Salvar alterações</button></div></GraphicJobActionForm></Card> : null}
    ...
    <Card title="Cotações de fornecedores">
      ...
      {quotes.length ? <div className="grid gap-4">{quotes.map((quote) => <article className="rounded-md border p-4" key={quote.id}>
        ...
        {canEditQuotes && quote.status === "pending" ? <div className="mt-4 flex gap-2"><Link className={secondaryButtonClassName} href={`/app/grafica/${id}?quote=${quote.id}`}>Editar</Link><RateLimitedActionForm action={cancelGraphicSupplierQuoteAction}><input name="id" type="hidden" value={quote.id} /><input name="jobId" type="hidden" value={id} /><button className={dangerButtonClassName} type="submit">Cancelar cotação</button></RateLimitedActionForm></div> : null}
