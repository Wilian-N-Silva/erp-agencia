...
## Usabilidade GRF-014 — fornecedores acessíveis na Gráfica — 22/09/2026

- Nova tela /app/grafica/fornecedores acessível pela listagem e pelo formulário de cotação. Reutiliza cadastro, formulário e mutações do Financeiro; não duplica fornecedores. Cadastro/edição/ativação preservam o histórico. Rótulos associados aos inputs permitem uso por teclado/leitores e seletores acessíveis.
- Nova permissão graphics.supplier_write autoriza somente manutenção dos fornecedores compartilhados. finance.configure continua válido; permissões de cotação isoladas permitem consultar, mas não alterar fornecedores. Nenhum acesso novo a contas, categorias, centros de custo ou liquidação.
- Migração 0043 gerada pelo drizzle-kit e aplicada nas bases local e de testes: catálogo/perfis Admin Técnico e Diretoria; demais concessões pelo RBAC existente. Sem tabela nova ou backfill de negócio. DAL mantém organização/RLS; mutações existentes continuam no wrapper transacional com auditoria e rate limit.
- Quatro testes de autorização/payload; E2E cadastra fornecedor pela Gráfica e usa-o na primeira cotação antes de rejeitar e seguir o fluxo. Upgrade ampliado até 0043. Gates: 370 unitários, 162 banco, E2E operacional, typecheck, lint e build aprovados. Erros iniciais do novo fixture unitário (mock e userId) corrigidos e suíte integral reexecutada.
- Ainda pendentes: navegação das pendências de importação, cobertura de saída histórica/pagamentos/multi-trabalho e auditoria final do módulo. Worktree sec-002 preservada; nenhuma promoção para development/main nesta etapa.

## GRF-013/014 — pendências navegáveis e cenários financeiros — 22/09/2026

- Alertas de linha importada agora abrem o lote na página e âncora corretas. Resolver o endereço exige graphics.import, UUID válido, organização e RLS; testes cobrem acesso negado/cross-tenant e linha 51 na segunda página.
- E2E ampliado para três blocos, dados inválidos ignorados com motivo, confirmação parcial gerando pendências, navegação pelo alerta, conclusão removendo pendência e relatório com proveniência. Duas vendas independentes recebem alocações explícitas de uma mesma entrada de R$ 800, sem duplicar caixa. Saída histórica permanece sem título inventado.
- E2E operacional inclui fornecedor cadastrado pela Gráfica, duas cotações/rejeição, cliente/OS/versionamento, contratação/AP, AR parcelada, sugestão/conciliação, produção/espera/entrega/encerramento e pagamento de R$ 1.200 conciliado com a AP. Saldos persistem após recarga.
- UI deixa de oferecer criação/edição/cancelamento de cotação após sair das etapas de fornecedores; servidor já impedia essas mutações. Migração 0044 gerada pelo Drizzle exige waiting_reason não nulo quando to_status=waiting, fechando a semântica NULL do CHECK; aplicada local/testes sem backfill, upgrade preserva histórico.
- Gates: typecheck, lint, build, 370 unitários, 163 testes de banco e 5 E2E aprovados. Verificação de detalhe a 390px sem overflow horizontal, screenshot inspecionado. E2E repetido usa limite de importação 30 somente no processo de teste; configuração padrão permanece 3/hora.
- Servidor local atualizado iniciado em localhost:3000 (PID 32092 na verificação); Docker PostgreSQL saudável. Inspeção manual no navegador confirmou dashboard e acesso à tela de fornecedores, deixada aberta para o usuário. Cinco clientes demo e fornecedores QA presentes. Conta demo existente continua válida.
- Ainda falta a auditoria final consolidada dos critérios/documentos e do ambiente antes de declarar todo o objetivo concluído. Nenhum merge efetuado nesta etapa; branch codex/graphics-completion, worktree sec-002 preservada.
